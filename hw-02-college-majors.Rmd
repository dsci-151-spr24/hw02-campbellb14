...
There could be outliers that will skew the data if the mean is used. 
...
Binwidth of 1000 was much smaller than binwidth 5000, so 5000 was a better choice because it showed the data more distinctly. 
...
The max and the min is helpful to show the range of the data. The max shows the peak, whereas the min shows the low point. The mean is also helpful because it shows where most of the data is distributed. The data is skewed to the right.
...
Interdisciplinary is the least popular in this sample. 
...
There are many more women in non-stem majors, but the women in stem majors make more money than non-stem majors. 

```{r}
scatterplot <- college_recent_grads %>%
  mutate(is_stem = ifelse(major_type == "STEM", "STEM", "Non-STEM"))

ggplot(college_recent_grads, aes(x = women, y = median, color = major_type)) +
  geom_point(size = 3, alpha = 0.7) +
  labs(title = "Median Income vs. Proportion of Women by Major",
...
Compare the number of men to the number of women in stem vs non-stem majors. 

```{r}
ggplot(college_recent_grads, aes(x = women, y = men, color = major_type)) +
  geom_point(size = 3, alpha = 0.7) +
  labs(title = "Men vs Women in Stem Major",
       x = "Women",
       y = "Men",
       color = "Major")
```
